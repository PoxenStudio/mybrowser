...
除下方 [SealSkin 兼容性](#sealskin-兼容性) 章节介绍的 SealSkin 通用环境变量外，本容器还支持以下专属环境变量：

|          变量           | 默认值   | 说明                                                                                                  |
| :----------------------: | :------: | ----------------------------------------------------------------------------------------------------- |
|       `CHROME_CLI`       |    无    | 浏览器启动参数，可以指定启动时打开的页面，也可以传入 Chrome 命令行参数。                                |
|        `UI_LANG`         | `zh-CN`  | 浏览器界面语言。                                                                                        |
| `ENV_REMOTE_INSPECT_PORT` |  未设置  | 设置后将以 `--remote-debugging-port=<端口>` 启动浏览器，开启远程调试。取值必须为 4 位及以上的整数且不超过 65535，否则该设置会被忽略。 |
